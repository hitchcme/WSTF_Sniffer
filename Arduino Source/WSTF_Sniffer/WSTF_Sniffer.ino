...
* obstacle, then a motion stop will be triggered via serial communication
to
...
* * * * * * * * * * * * * * * * * * * * * * * */
int n = 1; // counter
int decision = 0; // decision variable that determines direction after
periferal reading
...
Serial.begin(9600);
Wire.begin();
start = 0; // when first uploaded, queues initializatino phase before
reading
/*
...
frontSensor.setPulseReader(9); // initializes front sensor pulsewidth
reading
backSensor.setPulseReader(8); // initializes back sensor pulsewidth
reading
/*
...
// The "Mean" value will vary the whole loop beacuse it isalways
calculating the mean with the read values
// The "Last Mean" value will only show the calculated mean value just to
ease the reading of the calculated value
